...
n年期生存保险的精算现值记为$A_{x:\annu{n}^1}$或$_nE_x$

所以$A_{x:}\frac{1}{\annu{n}}\equiv {}_nE_x=v^n\cdot{}_np_x$
\begin{itemize}
...
${}_{m|}\overline{A}_{x:\annu{n}}^1=E(Z)=\sum_{k=m}^{m+n-1}v^{k+1}{}_{k|}{q}_{x}$
...
$0 \leq S(x)<1$,$0\leq mS(x)<m$, 令$J(x)=[mS(x)]$, 它表示$mS(x)$的整数部分, 于是有$0\leq J(x)<m-1$, 易知$J(x)\leq mS(x)<J(x)+1$. 从而$\frac{J(x)}{m}\leq S(x)\leq\frac{J(x)+1}{m}$, $K(x)+\frac{J(x)}{m}\leq K(x)+S(x)\leq K(x)+\frac{J(x)+1}{m}$.

\begin{itemize}
    \item[{\bf\hei 1.}]{\bf\hei 即$K(x)+\frac{J(x)}{m}\leq T(x)\leq K(x)+\frac{J(x)+1}{m}$}
\end{itemize}
...
\begin{align*}
    A_{x}^{(m)} & =E(Z)                                                                                            \\
                ...
                & =\sum_{k=0}^{\infty}\sum_{j=1}^{m-1}v^{k+\frac{j+1}{m}}P(K(x)=k,J(x)=j)                          \\
                & =\sum_{k=0}^{\infty}\sum_{j=1}^{m-1}v^{k+\frac{j+1}{m}}P(k+\frac{j}{m}\leq T(x)<k+\frac{j+1}{m}) \\
                & =\sum_{k=0}^{\infty}\sum_{j=1}^{m-1}v^{k+\frac{j+1}{m}}{}_{k+\frac{j+1}{m}|\frac{1}{m}}q_{x}
\end{align*}

在UDD假设下, $A_{X}^{(m)}$与$\overline A_{x}$, $A_{x}$之间的关系:
\begin{proposition}
    在UDD假设下, $K(x)$与$J(x)$独立, 且$P(J(x)=1)=\frac{1}{m}$, $j=0, 1, ..., m-1.$
\end{proposition}

\proof
在UDD假设下, $K(x)$与$S(x)$独立, 且$S(x)\sim U(0,1)$\newline
所以$K(x)$与$[mS(x)]=J(X)$独立. \newline
又因$S(x)\sim U(0,1)$,所以J(X)取$0, 1, \dots, m-1$的概率相同, 皆为$\frac{1}{m}$.
\qed
...
\begin{itemize}
    \item[{\bf\hei 4.}]{\bf\hei 当$b_{k(x)+1}$时, 得到标准递增的死亡年末支付的终身寿险}
\end{itemize}
...
(2)精算现值记为$1A_{x}$:
\begin{align*}
    1A_{x} & =\sum_{k=0}^{\infty}b_{k+1}v^{k+1}{}_{k|}q_{x}               \\
...
即\begin{align*}
    1A_{x}=\sum_{k=0}^{\infty}(k+1)v^{k-1}{}_{k|}q_{x}=\sum_{j=0}^{\infty}{}_{j|}A_{x}
\end{align*}
...
\begin{itemize}
    \item[{\bf\hei 4.}]{\bf\hei 若\begin{align*}
                    b_{T}(x)=\sum_{k=0}^{\infty}(k+1)I_{\{k\leq T(x)<k+1\}}
                \end{align*}
                ,即在第(k+1)年死亡, 则死亡时立即支付k+1元, 则得到标准递增的死亡立即支付的终身寿险. }
\end{itemize}
...
\begin{align*}
    (I\overline{A})_{x} & =E(Z)                                                                                                 \\
                        ...
                        & =\sum_{k=0}^{\infty}\left(k+1\right)E\left(v^{T(x)}\right]_{\{k\leq T\left(x\right)\leq k+1\}}        \\
                        & =\sum_{k=0}^{\infty}\left(k+1\right)\int_{k}^{k+1}v^{t}{}_{t}P_{x}\mu_{x}(t)dt
\end{align*}
\begin{itemize}
    \item[{\bf\hei 5.}]当$b_T(x)=T(x)$时，得到{\bf\hei 连续递增}的{\bf\hei 死亡立即支付}的终身寿险
\end{itemize}
...
\end{itemize}
\begin{align*}
    \ddot a_{x:}{}_{\annu n} & E(\ddot a_{\annu {(K(x)+1)\land n}})=E(\sum_{j=0}^{(K(x)+1)\land n}{v^j})=\sum_{j=0}^{K(x)\land(n-1)}{v^j}                               \\
                             & =E(\sum_{j=0}^{\infty}{v^j}I_{\{j\leqslant K(x)\land(n-1)\}})=\sum_{j=0}^{\infty}{v^j}E(I_{\{j\leqslant K(x)\}}I_{\{j\leqslant (n-1)\}}) \\
...
所以有$\ddot a_{x:}{}_{\annu n}=\frac{1-A_{x:}{}_{\annu n}}{d}$,所以(1)得证
\begin{align*}

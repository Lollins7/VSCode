...
\begin{exs}设系统中有$l_0$个新生儿, 它们的寿命独立同分布, 生存函数为
    64    $$s(t)=1-\frac{t}{16}, 0\le t\le 16.$$ 证明$$({}_4\mathscr D_0, {}_4\mathscr D_4,{}_{4}\mathscr D_8, {}_{4}\mathscr D_{12})$$服从多项分布, 并计算 (1), 每个随机变量的期望; (2) 每个随机变量的方差; (3) 每两个随机变量的相关系数; (4) 对你计算所得的结果进行简要分析.
\end{exs}
\begin{exs}
...
\begin{exs}
    ...
    (1) 设$\mu_{x}(t)\equiv \mu =0.05,\ \delta\equiv 0.04$, 求：$E(v^{T(x)});D(v^{T(x)});E(\int_{0}^{T(x)}v^{t}dt);D(\int_{0}^{T(x)}v^{t}dt)$;

    (2) 设UDD成立, 求证：$E(v^{T(x)})=\frac{T}{\delta}E(v^{k(x)+1})$.
\end{exs}
...
\end{itemize}
$$
    \text{人寿保险}\left\{ \begin{array}{c}
        \text{生存保险}                                                                 \\
        \text{生死合险}                                                                 \\
        \text{死亡保险}\left\{ \begin{array}{c}
                               \text{定期死亡保险}                            \\
                               \text{终身死亡保险}                            \\
                               \text{延期死亡保险}\left\{ \begin{array}{c}
                                     \text{延期定期死亡保险} \\
                                     \text{延期终身死亡保险} \\
                                 \end{array} \right. \\
                           \end{array} \right. \\
...
若被保险人在$n$年内死亡(即$T(x)<n$), 则不予任何支付; 若他在n年内未死(即$T(x)\geqslant n$), 则在$n$时刻支付他1元保险金.
\begin{itemize}
...
n年期生存保险的精算现值记为$\overline{A}_{x:}\frac{1}{\annu{n}}$或$_nE_x$

所以$\overline{A}_{x:}\frac{1}{\annu{n}}\equiv _nE_x=v^n\cdot _np_x$
\begin{itemize}
...
\begin{remark}
    \begin{enumerate}
        \item 等式$(1+i)^k\cdot l_x\cdot {}_nE_x=l_{x+k}\cdot {}_{n-k}E_{x+k}$的含义是：
...
死亡年末支付——离散型
\begin{itemize}
    \item[{\bf\hei 一.}]{\bf\hei 死亡立即支付的定期寿险}
\end{itemize}
\begin{itemize}
...
\begin{remark}
    ...
    $$
        \overline{A}_{x:\annu{n}}^1=EZ=E\left( v^{T\left( x \right)}I_{\left\{ T\left( x \right) <n \right\}} \right) =\int_0^{\infty}{v^{t}I_{\left\{ T\left( x \right) <n \right\}}\cdot f_{T(x)}(t)dt}
    $$
    $$
...
\begin{align*}
    E(Z^2) & =E((v^{T(x)}I_{T(x)<n})^2)=E(v^{2T(x)}I_{T(x)<n})                                  \\
           & =E(e^{-2\delta T(x)}I_{T(x)<n})=\int_0^{\infty}{e^{-\delta t}I_{t<n}f_{T(x)}(t)dt} \\
...
{\rm\bf(i)} $A_{x:\annu{n}}^1=A_{x:}^1{}_{\annu m}+{}_mE_x\cdot A_{x+m:}^1{}_{\annu {n-m}}~,\forall~0\leq m\leq n$

特别地, 当$m=1$时, $A_{x:\annu{1}}^1 = vq_x,~E_x = vp_x$.

    {\rm\bf(ii)} $A_{x:\annu{n}}^1=vq_x+vp_x\cdot A_{x:\annu{n-1}}^1$

{\rm\bf(iii)} $(1+i)l_x A_{x:\annu{n}}^1 = d_x + l_{x+1} A_{x+1:\annu{n-1}}^1$
...
{\rm\bf(i)} $\overline{A}_x = \overline{A}_{x:\annu{n}}^1 + {}_nE_{x}\cdot \overline{A}_{x+n}$

{\rm\bf(ii)} $\frac{d\overline{A}_x}{dx} = \delta \overline{A}_x + \mu(x)(\overline{A}_x - 1)$
...
\section{作业}
\begin{exs}
    解释等式$l_x\overline{A}_{x:\annu{n}}^1=l_x\overline{A}_{x:\annu{n}}^1+v^kl_{x+k}\overline{A}_{x:\annu{n}}^1$的含义.
\end{exs}
